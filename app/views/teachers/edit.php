...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Edit Teacher Profile</h2>
        
        <form action="<?php echo URLROOT; ?>/teachers/edit/<?php echo $data['id']; ?>" method="POST" class="space-y-6">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-3">Update Subject Assignments</label>
                    ...
                    <?php foreach($data['all_subjects'] as $subject) : ?>
                        <label class="flex items-center p-3 rounded-lg border border-gray-100 bg-gray-50 hover:bg-blue-50 cursor-pointer transition-colors group">
                            <input type="checkbox" name="subjects[]" value="<?php echo $subject->id; ?>" 
                                   <?php echo in_array($subject->id, $data['subjects']) ? 'checked' : ''; ?>
                                   class="rounded text-blue-500 focus:ring-blue-500 mr-3">
